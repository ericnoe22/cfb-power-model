...
# ── Page: Betting Edges ────────────────────────────────────────────────────
    ...
    st.caption("Lines auto-refreshed every 30 min — click Refresh to pull latest now")
    ...
    if not _sched_lines.empty:
        from data.team_names import normalize as _norm
        _want = {"homeTeam", "awayTeam", "spread", "overUnder", "home_ml", "away_ml"}
        _cols = [c for c in _want if c in _sched_lines.columns]
        vegas_df = _sched_lines[_cols].rename(columns={"spread": "vegas_spread", "overUnder": "vegas_total"})
